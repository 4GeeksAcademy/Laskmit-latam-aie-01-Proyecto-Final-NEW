# Proyecto: Building Bullet-Proof Applications

## Descripción general:

La API de autenticación de la empresa está en producción, gestionando usuarios y sesiones reales. La semana pasada, un compañero subió un pequeño refactor que rompió la lógica de expiración de tokens — ninguna prueba lo detectó, y los usuarios reportaron estar bloqueados durante dos horas antes de que alguien se diera cuenta. "Necesitamos una batería de pruebas. El código sin tests no es código de producción."
Debemos añadir una batería completa de pruebas unitarias a la API de autenticación que construimos en el hito anterior. Debemos trabajar a nivel de la lógica de funciones y endpoints — no probando la serialización HTTP ni las tuberías del framework, sino la lógica de negocio real: ¿se genera correctamente el token? ¿Se rechaza un token expirado? ¿Qué ocurre cuando el campo de contraseña está vacío?
Se trata de construir la confianza de que cada endpoint se comporta como se espera en condiciones normales, en casos límite y en escenarios de fallo — los tres pilares de cualquier plan de pruebas serio.

**Cobertura de pruebas unitarias para la API de autenticación**
Prioridad: Alta
...
Antes de escribir la primera prueba, debemos definir solamente un plan de pruebas: lista los casos que quieres cubrir para cada endpoint, identifica los casos límite (campos vacíos, usuarios duplicados, tokens malformados) y piensa qué entradas podrían producir comportamientos inesperados. 
Luego de revisar, te diré para generar el código de los tests.
...
### PASO 01: Plan de pruebas
[ ] Crea un archivo TESTING.md en la raíz del proyecto documentando: cómo ejecutar las pruebas, qué cubre cada suite y qué casos se deben incluir y por qué.
...
### PASO 02: Escribir código de pruebas
...
### PASO 03: MEJORAR EL FLUJO DE TRABAJO
...
IMPORTANTE: No pruebes la serialización HTTP ni los internos del framework. Cada test debe afirmar algo sobre la lógica de negocio de tu aplicación — lo que el endpoint decide, no cómo responde.

### PASO 04: ACTIVIDAD EXTRA

Hay dos tickets que llevan semanas en el backlog sin que nadie los persiga. Son de baja prioridad — nadie los está pidiendo — pero ahora que tienes la infraestructura de testing montada, sería una pena no cerrarlos. Si terminas antes de tiempo, liquídalos.
...
## VERIFICACIÓN FINAL DEL TRABAJO EFECTUADO PARA ESTE PROYECTO
